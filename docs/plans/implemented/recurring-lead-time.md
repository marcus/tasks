...
Status: **implemented** (2026-08-01), including the optional clock units.

Tracking epic: td-f18c31 (stories td-190c8e, td-b93664, td-b7b218, td-cbca83,
td-6d7763, td-556c53, td-526a45).

Proof: [`docs/proofs/lead-time.md`](../../proofs/lead-time.md).

Deltas between this contract and the shipped code, all deliberate and reviewed:

- **Extra guard, not a relaxation.** A lead whose derived date would fall
  outside the storable four-digit year range is refused at write time, the same
  guard recurrence already has. It is not one of the five rules below.
- **Superset on the wire.** The resource carries `lead_human`, `lead_opens`, and
  `lead_opens_at` beside `lead`. `lead_opens_at` is what makes a clock lead
  legible at all (its window opens partway through a day), and deriving the
  window once in `TaskQueries` is what keeps every adapter from re-deriving it.

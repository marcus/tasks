...
| `archive` | `conflict` (with `reason`: `open_descendants`, `archive_conflict`, `preview_changed`, `write_failed`), `schema_migration_required` |
| `undo`, `redo` | `empty`, `conflict`, `schema_migration_required` |
...
| `archive` | ✅ | `{roots, records, moved_ids}` — `roots` is what the human line counts, `records` the whole swept subtree (what `moved_ids` lists). Deliberately not named `archived`: the sibling `project archive --json` uses that word for its record count. Refusals: `conflict` with `reason` = `open_descendants` (carrying `blocked` + `open_descendants`), `archive_conflict` (carrying `conflicting_ids`), `preview_changed` (the store changed while the sweep was being prepared — retry), or `write_failed`; `schema_migration_required` on a v1 store. |
| `undo` | ✅ | `{action: "undo", label}`; errors `empty`, `conflict`, `schema_migration_required` |
| `redo` | ✅ | `{action: "redo", label}`; errors `empty`, `conflict`, `schema_migration_required` |
| `migrate` | ✅ | `{status, from_version, to_version, files, backups, errors}` |
...
| v1 store | `error: schema_migration_required` | `code: schema_migration_required` |

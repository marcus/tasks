...
| `next` | `n` | ✅ | NEXT actions by context. With none, prints the one-line empty state `No next actions. Dated work is on the agenda; mark one with: tasks state <ref> NEXT` — dating lands work in `TODO`, so an empty NEXT list usually sits beside a full agenda and must say so rather than print nothing. `--json` is unaffected (an empty array). |

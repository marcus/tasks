...
`#` (and the Delete key) hard-deletes the selected live task — the TUI surface
of `tasks delete`. It always confirms first: a leaf asks once, a parent with
descendants opens a cascade confirm that names counts (children are removed, not
reparented). Recurring tasks are removed entirely rather than rolled forward.
`n` or Escape cancels without writing; after a successful delete, `u` / ctrl-r
undo and redo through the shared journal.
...
`show`, and the approval section of the final TUI Inbox tab. The proposed scope
is a decision QUEUE, so it is handed over in the shared **triage order** —
priority `A` > `B` > `C` > none, then the soonest deadline-or-scheduled boundary
inside a band, undated after dated inside that band, canonical file/DFS order
breaking a full tie. Priority strictly dominates the date: a `C` due tomorrow
still sorts below an undated `A`. The same order applies to `list --proposed`
(text and `--json`), the TUI Inbox Approvals section, and
`GET /api/v1/tasks?scope=proposed`, because all three call one core function
...
**The transition stamp is stored in UTC and read locally.** `delegation.at` is
written as an RFC 3339 UTC instant (`2026-08-28T00:46:44Z`) and keeps that
spelling on disk, in every `--json`, and in every API payload — a machine
reader must never have to guess the viewer's zone to compare two claims. Every
**human** print of it projects into the configured `timezone` and `time_format`,
...
a field a person can see and question beats a field that quietly vanished.
...
| `links` | ✅ | `{links: [{url, label, system, link_source, task, id, line, source}]}`; `link_source` is `formal`, `title`, or `body`; the established task `source` remains `live` or `archive`. |
| `open` | ✅ | `{id, line, title, url, label, system, source, opened}` — `opened` is false under `--print`. Errors: `not_found` (no links / no such index), `ambiguous` (several links, carrying `links`), `unavailable` (no browser launcher). |
| `id` | ✅ | `{id, touched: [task]}` |

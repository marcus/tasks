...
Implementation note: the transport itself lands in Phase 4 of
`docs/plans/http-api-and-web-foundation.md`. This ADR records the accepted
dependency and runtime decision that the Phase 0 contract
(`docs/api/openapi.yaml`) is written against; the Gemfile, `config.ru`,
`bin/tasks-api`, and the Rack app are added when Phase 4 begins.
...
- A committed `Gemfile` and `Gemfile.lock` scope the runtime dependency set to
  Rack and Puma. Test-only gems — `rack-test` and an OpenAPI validator such as
  `committee` — live in the Gemfile's test group and do not extend the runtime
  dependency set.

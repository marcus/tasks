...
protect_detached_commit() { # protect_detached_commit <slot> <wt>
  ...
  if ! git -C "$wt" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    log "slot=$slot FAILED to inspect worktree $wt; detached commit safety is unknown"
    return 1
  fi
  git -C "$wt" symbolic-ref -q HEAD >/dev/null 2>&1 && return 0
  head="$(git -C "$wt" rev-parse -q --verify HEAD 2>/dev/null)" || {
    log "slot=$slot FAILED to inspect HEAD in $wt; detached commit safety is unknown"
    return 1
  }

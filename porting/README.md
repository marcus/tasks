...
|---|---|---|
| `manifest.jsonl` | one record per proof-sized capability: id, deps, risk, `source_sha`, fixtures, status, evidence locator | td-940935 |
...
- **Progress** — `manifest.jsonl` and `evidence/`, never a hand-written
  percentage and never a td query.
...
The fleet has nothing to do until the manifest is seeded. Read
[`OPERATING.md`](OPERATING.md) before starting it for real.

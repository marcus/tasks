...
// Contract is the source name of the versioned list-agent contract embedded in
// every binary. Installed builds must not depend on a source checkout.
const Contract = "TASK_AGENT.md"

//go:embed TASK_AGENT.md
var embeddedContract string
...
// Build assembles the whole context. cliExecutable is the installed tasks
// command — distinct from the task-data directory the harness runs in.
func Build(paths config.Paths, cliExecutable string, sources promptfacts.Sources) (string, error) {
	base := embeddedContract
	...
	sections := []string{base, facts, FileLocations(paths, cliExecutable), MemoryPointer, memory}
	kept := sections[:0]
	for _, section := range sections {
		...
		// the contract and after the file locations. Keep that stable for prompt
		// caches and every provider adapter.
...
//
// The CLI it names is the resolved executable path. That keeps agent prompts
// correct for Homebrew, make install, symlinks, and development builds.
func FileLocations(paths config.Paths, cliExecutable string) string {
	return fmt.Sprintf(
		"File locations for this run (absolute; use these, not relative paths):\n"+
			...
			"- archive.jsonl: %s\n"+
			"- agent-memory.md: %s\n",
		cliExecutable, paths.Org, paths.Archive, paths.Memory)
}

...
func TestProductionResolutionRequiresADirectoryOrBothPerFilePaths(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "only tasks env", overrides: map[string]string{"TASKS_FILE": "/data/tasks.jsonl"}, want: false},
		{name: "only archive env", overrides: map[string]string{"TASKS_ARCHIVE": "/data/archive.jsonl"}, want: false},
		{name: "both env paths", overrides: map[string]string{"TASKS_FILE": "/one/tasks.jsonl", "TASKS_ARCHIVE": "/two/archive.jsonl"}, want: true},
		{name: "directory env", overrides: map[string]string{"TASKS_DIR": "/data"}, want: true},
		{name: "only tasks config", config: "file = /data/tasks.jsonl\n", want: false},
		{name: "only archive config", config: "archive = /data/archive.jsonl\n", want: false},
		{name: "mixed complete pair", overrides: map[string]string{"TASKS_FILE": "/one/tasks.jsonl"}, config: "archive = /two/archive.jsonl\n", want: true},
		{name: "directory config", config: "dir = /data\n", want: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			env, _ := testEnv(t, tc.overrides)
			if tc.config != "" {
				writeConfig(t, env, tc.config)
			}
			if got := Resolve("", env, func() string { return "host" }).Configured; got != tc.want {

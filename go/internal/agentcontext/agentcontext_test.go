...
func newSandbox(t *testing.T) *sandbox {
	...
	cliRoot := t.TempDir()
	dataDir := t.TempDir()
	write(t, filepath.Join(cliRoot, Contract), "# TASK_AGENT\nContract prose here.\n")
	return &sandbox{
		cliRoot: cliRoot,
		dataDir: dataDir,
...
func (s *sandbox) build(t *testing.T) string {
	t.Helper()
	context, err := Build(s.paths, s.cliRoot, s.sources())
	if err != nil {
...
func (s *sandbox) buildErr(t *testing.T) error {
	t.Helper()
	if _, err := Build(s.paths, s.cliRoot, s.sources()); err != nil {
		return err
	}
...
func TestIncludesAgentsContractAndAbsolutePaths(t *testing.T) {
	...
	for _, needle := range []string{
		"Contract prose here.",
		filepath.Join(s.cliRoot, "bin", "tasks"),
...
func TestIncludesCurrentEnvironmentBeforeFileLocations(t *testing.T) {
	...
	contractAt := strings.Index(context, "Contract prose here.")
	factsAt := strings.Index(context, "Current environment:")
	pathsAt := strings.Index(context, "File locations for this run")
...
func TestMissingContractFileStillBuildsFromPathsAndPointer(t *testing.T) {
	s := newSandbox(t)
	if err := os.Remove(filepath.Join(s.cliRoot, Contract)); err != nil {
		t.Fatalf("remove: %v", err)
	}
	context := s.build(t)

	if strings.Contains(context, "Contract prose here.") {
		t.Fatalf("the contract survived its own deletion:\n%s", context)
	}
	if !strings.Contains(context, filepath.Join(s.dataDir, "tasks.jsonl")) {
		t.Fatalf("the file locations went missing:\n%s", context)
	}
	if !strings.Contains(context, MemoryPointer) {
		t.Fatalf("the memory pointer went missing:\n%s", context)
	}
	// And nothing leads with a blank section: an empty contract drops out
	// entirely rather than leaving the document starting on a newline.
	if strings.HasPrefix(context, "\n") {
		t.Fatalf("a dropped section left its separator behind: %q", context[:20])
	}
}
...
func TestSectionsJoinWithABlankLineAndKeepTheirOwnTrailingNewline(t *testing.T) {
	...
	want := "# TASK_AGENT\nContract prose here.\n" + "\n\n" +
		FileLocations(s.paths, s.cliRoot) + "\n\n" + MemoryPointer
	if context != want {
		t.Fatalf("context =\n%q\nwant\n%q", context, want)
	}
}
...
func TestFileLocationsNamesTheInstalledCLIAndEveryFile(t *testing.T) {
	s := newSandbox(t)
	block := FileLocations(s.paths, s.cliRoot)
	want := "File locations for this run (absolute; use these, not relative paths):\n" +
		"- tasks CLI: " + filepath.Join(s.cliRoot, "bin", "tasks") + "\n" +
...
func TestFileLocationsFollowsARelocatedMemorySidecar(t *testing.T) {
	...
	if !strings.Contains(FileLocations(s.paths, s.cliRoot), "- agent-memory.md: "+elsewhere+"\n") {
		t.Fatalf("block = %q", FileLocations(s.paths, s.cliRoot))
	}
}

// A whole-file section is included verbatim, so a contract that ends without a
// newline does not silently gain one.
func TestContractIsIncludedVerbatim(t *testing.T) {
	s := newSandbox(t)
	write(t, filepath.Join(s.cliRoot, Contract), "no trailing newline")
	if !strings.HasPrefix(s.build(t), "no trailing newline\n\n") {
		t.Fatalf("context = %q", s.build(t))
	}
}

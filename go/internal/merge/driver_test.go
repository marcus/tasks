...
// A stated, recorded difference rather than an accident: bin/tasks dispatches
// merge-driver BEFORE it resolves a task store, so the driver depends only on
// the four paths Git supplied. The Go dispatcher resolves configuration first,
// so an invalid TASKS_TIMEZONE adds one resolution note to the driver's stderr
// that Ruby never prints. Nothing about the merge changes — same exit status,
// same bytes in %A, same audit log — and moving the dispatch earlier is an edit
// to cmd/tasks/main.go, which belongs to the integration owner.
//
// See porting/intentional-differences.md,
// merge-driver-runs-after-config-resolution.
func TestTheGoDriverStillResolvesConfigurationFirst(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "ignoring invalid time zone") {
		t.Skip("dispatch now precedes configuration resolution; retire this entry")
	}
}

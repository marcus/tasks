...
func TestFloatSpellingsMatchTheRubyOracle(t *testing.T) {
	...
	raw, err := os.ReadFile(filepath.Join(evidence, "ruby", "float-spellings.json"))
	if err != nil {
...
// recorded Ruby oracle: porting/evidence/format-canonical-emit/oracle-capture.md
...
func TestFixtureDumpsMatchRubyBytes(t *testing.T) {
	...
	for _, testCase := range cases {
		t.Run(testCase.path, func(t *testing.T) {
			input, err := os.ReadFile(filepath.Join("..", "..", "..", "porting", "fixtures", testCase.path))
			if err != nil {

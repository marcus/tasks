...
// bin/tasks aborts on a valued flag whose value is missing, and a command that
// forgot that turns the NEXT positional into the flag's value and writes it.

// takeFlagValue is bin/tasks' extract_value: pull `--flag <value>` out of the
...
// whole reason bin/tasks orders it that way too.
...
// parseTemporalArg is bin/tasks' parse_temporal: the friendly expression, or an
...
// patchBaseline is bin/tasks' `snapshot && patch_expected(snapshot, field)`.
//
